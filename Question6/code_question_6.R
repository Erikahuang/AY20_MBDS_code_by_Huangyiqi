...
output_inside$V3<-"inside"
# outside points
output_outside<-input_points[input_points$V3%%2==0,]
output_outside$V3<-"outside"
output<-rbind(output_inside,output_outside)
write.table(output,file="C:/Users/Administrator/Desktop/AY20_MBDS_questions/Question 6/output_question_6.txt",
            row.names=F,col.names=F,quote=F,sep="\t")

## results plot
plot(input_polygon, type="b",xlim=c(0,20),ylim=c(0,25),xlab="x",ylab="y")
par(new=T)
plot(output_inside$V1,output_inside$V2, col="red",axes=F,xlim=c(0,20),ylim=c(0,25),xlab="x",ylab="y")
par(new=T)
plot(output_outside$V1,output_outside$V2, col="blue",axes=F,xlim=c(0,20),ylim=c(0,25),xlab="x",ylab="y")
par(new=F)
